...
def test_gradients_agree_at_the_ols_minimum() -> None:
    ...
    # gradient itself is large. OLS.fit centers y internally (see
    # regression.base.LinearModel), so the gradient must be evaluated
    # against that same centered target, not the raw y.
    ...
    analytical = analytical_gradient(X, y_centered, theta)
    autodiff = autodiff_gradient(X, y_centered, theta)
