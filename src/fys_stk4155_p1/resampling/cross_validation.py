...
def kfold_mse_degree_sweep(
    ...
    """K-fold cross-validated test MSE, per polynomial degree.
    ...
    cross_val_score` over a `KFold(k, shuffle=True)` split. Scaling
    ...
    Returns:
        Dict with keys "degrees", "mse_mean", "mse_std" (arrays aligned with
        "degrees"); "mse_std" is the standard deviation of the k per-fold MSEs.
    """
    ...
    for i, degree in enumerate(degrees_arr):
        ...
        mse_mean[i] = -scores.mean()
        mse_std[i] = scores.std()

    return {"degrees": degrees_arr, "mse_mean": mse_mean, "mse_std": mse_std}
