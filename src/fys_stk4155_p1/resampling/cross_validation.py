...
def _make_pipeline(degree: int, model_factory: Callable[[], LinearModel]) -> Pipeline:
    ...
    Every column (x^1..x^degree; there is no intercept column -- the model
    itself fits the intercept by centering y, see `regression.base.
    LinearModel`) is standardized by a `StandardScaler` fit inside the
    pipeline, i.e. on the training fold only. Degree 0 has no columns to
    scale at all (StandardScaler rejects a zero-column input), so the scaler
    is omitted for that case.
    """
...
def kfold_mse_degree_sweep(
    ...
    For each degree, builds the polynomial design matrix and scores
    `model_factory()` with `sklearn.model_selection.cross_validate` over a
    `KFold(k, shuffle=True)` split. Scaling (`StandardScaler` on every
    column) is done inside a `Pipeline`, so it is fit on each training fold
    only, never on the held-out fold or the full dataset.
